...
const _functions = {
  ...
  add: function() {
    ...
    if (data.todo) {
      $.post("/api/add", data).then(function(action) {
        if (action.success) {
          _functions.render(_functions.makeNewItem(state.inputBar.val(), 0));
          state.inputBar.val("");
        }
      });
    }
